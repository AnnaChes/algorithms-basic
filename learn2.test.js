...
describe('Section for JavaScript algorithms', () => {
  ...
  test('Function to return people who "meet at the class"', () => {
    ...
    const getPairArr = arr => {
      ...
      arr.forEach((item, i, arr2) => {
        arrTemp.shift()
        arrTemp.forEach(itemShifted => {
          output.push([item, itemShifted])
        })
      })
      // console.info('getPairArr [10]', { arr, output })
      return output
    }
    ...
    const findPairs = input => {
      ...
      const inputInt = input.map(item => [parseInt(item[0], 10), item[1]])
      // console.info('findPairs [1]', { inputInt: JSON.stringify(inputInt) })
      const inputSort = inputInt.sort((a, b) => a[0] - b[0])

      // Create a separate array of id and create array of pairs
      const inputId = inputSort.map(item => item[0])
      const inputIdFiltered = inputId.filter((item, i, arr) => {
        let output = false
        if (i === 0 || (arr[i - 1] !== undefined && arr[i - 1] !== item)) {
          output = true
        }
        return output
      })
      const pairArr = getPairArr(inputIdFiltered)
      // console.info('findPairs [2]', { pairArr, inputId, inputIdFiltered, inputSort })

      // Distribute values to pairs
      const answer = []
      pairArr.forEach(pair => {
        inputSort.forEach(itemInput0 => {
          inputSort.forEach(itemInput1 => {
            if (itemInput0[0] === pair[0] && itemInput1[0] === pair[1]) {
              if (itemInput0[1] === itemInput1[1]) {
                answer.push([pair, [itemInput0[1]]])
              }
              else {
                answer.push([pair, []])
              }
      ...
      // console.info('findPairs [4]', { answer: JSON.stringify(answer) })
      const answer1 = {}
      answer.forEach(item => {
        const key = JSON.stringify(item[0])
        if (!answer1[key]) {
          answer1[key] = [...item[1]]
        }
        else if (answer1[key] && item[1].length !== 0) {
          answer1[key] = [...answer1[key], ...item[1]]
        }
      })

      // Transform object back to array
      const answer2 = Object.keys(answer1).map(key => {
        const id = JSON.parse(key)
        return [id, answer1[key]]
        // console.info('findPairs [7]', { key: JSON.parse(key) })
      })
    ...
    const output_1E = findPairs(student_course_pairs_1)
    expect(JSON.stringify(output_1E)).toBe(JSON.stringify(output_1))
    ...
    const output_2E = findPairs(student_course_pairs_2)
    expect(JSON.stringify(output_2E)).toBe(JSON.stringify(output_2))

    const student_course_pairs_3 = [
      ["58", "Software Design"],
      ["58", "Linear Algebra"],
      ["94", "Art History"],
      ["94", "Operating Systems"],
    ...
    const output_3E = findPairs(student_course_pairs_3)
    // console.info('findPairs [10]', { output_3E: JSON.stringify(output_3E) })
    expect(JSON.stringify(output_3E)).toBe(JSON.stringify(output_1))

  })
  // Description: https://gist.github.com/jmetev1/f4a1fab9a12b7cfbb81fbdf7ddc06310
  ...
  test('22.3 Simulating querySelector method with document.', () => {

    const domCollectionToElement = entity => {
      let el = entity
      if (entity && !!entity.length === true) {
        el = document.createElement('div')
        for (let i = 0; i < entity.length; i += 1) {
          el.appendChild(entity[i])
        }
      }
      return el
    }

    const stringifyEl = (elem, truncate) => {
      const truncateLen = truncate || 50

      let el = domCollectionToElement(elem)

      // eslint-disable-next-line prefer-destructuring
      let ret = el && el.innerHTML
      if (ret) {
        ret = ret.substring(0, truncateLen)
      }

      // If we've truncated, add an elipsis.
      if (ret && ret.length > truncateLen) {
        ret += '...'
      }
      return ret
    }
    ...
    const logic = (elem, query) => {
      let elemNext
      const queryNext = query.slice(1)

      // console.info('22.3 logic 2', { queryNext, query, elem, elemStr: stringifyEl(elem, 1000), collection: !!elem.length })

      if (query.includes('#')) {
        document.body.appendChild(elem)
        elemNext = document.getElementById(queryNext)
      }
      else if (elem && query.includes('.')) {
        // console.info('22.3 logic [3] getElementsByClassName')
        // eslint-disable-next-line prefer-destructuring
        elemNext = elem.getElementsByClassName(queryNext)
        // console.info('22.3 logic [7] getElementsByClassName', { queryNext, elem })
      }
      else if (elem && !query.includes('#') && !query.includes('.')) {
        // console.info('22.3 logic [5] getElementsByTagName')
        // eslint-disable-next-line prefer-destructuring
        elemNext = elem.getElementsByTagName(query)
      }
      else {
        return null
      }

      elemNext = domCollectionToElement(elemNext)
      // console.info('22.2 logic [10]', { queryNext, query, elemNext, elemNextStr: stringifyEl(elemNext, 1000), collection: elemNext && !!elemNext.length })
      return elemNext
    }

    const walker = (elemObjIn, queryStrIn, iIn = 0) => {
      // console.info('walker [0]', { elemObjIn: stringifyEl(elemObjIn, 1000), collection: !!elemObjIn.length, queryStrIn, iIn })
      const queryArrIn = queryStrIn.slice().split(' ')
      let elemObj = elemObjIn
      if (elemObjIn && elemObjIn.body) {
        elemObj = elemObjIn.body
        // console.info('walker [1-1]', { elemObj: stringifyEl(elemObj, 1000), collection: !!elemObj.length, elemObjIn })
      }
      else if (elemObjIn && !!elemObjIn.length === true) {
        elemObj = domCollectionToElement(elemObjIn)
        // console.info('walker [1-2]', { elemObj: stringifyEl(elemOb, 1000), collection: !!elemObj.length, elemObjIn })
      }

      // console.info('walker [2]', { 'elemObj': stringifyEl(elemObj, 1000) })
      const logicOutput = logic(elemObj, queryArrIn[iIn])
      // console.info('walker [3]', { iIn, logicOutput, logicOutputStr: stringifyEl(logicOutput, 1000), collection: logicOutput && !!logicOutput.length, elemObj, queryArrIn })

      if (logicOutput && iIn < queryArrIn.length - 1) {
        // console.info('walker [9]', { iIn, 'queryArrIn[i]': queryArrIn[iIn], children, queryStrIn, elemObj })
        return walker(logicOutput, queryStrIn, iIn + 1)
      }
      // console.info('walker [10]', { iIn, 'queryArrIn[i]': queryArrIn[iIn], children, queryStrIn, elemObj })
      return logicOutput
    }

    const elemStr = '<div id="div_00"><div id="div_10" class="class_10"><div id="div_20"></div><div id="div_21"><div id="div_30"></div><div id="div_31"></div><div id="div_32"></div></div></div></div>';
    document.body.innerHTML = elemStr
    // const queryArr = ['#div_00'] div .class_10 #div_21
    const queryStr = 'div .class_10 #div_21'
    const result = walker(document, queryStr)

    // console.info('22.3 Simulating querySelector [10-2]', { result: stringifyEl(result, 1000), queryStr })

    const answer = '<div id="div_30"></div><div id="div_31"></div><div id="div_32"></div>'
    expect(result.innerHTML).toBe(answer)
  })
  // Create a function to simulate querySelector of a DOM tree with
  // document.getElementById(id)	Find an element by element id
  // document.getElementsByTagName(name)	Find elements by tag name
  // document.getElementsByClassName(name)	Find elements by class name


  test('22.2 Top level simulating querySelector method with array', () => {
    ...
    const queryArr = ['div_00', 'div_10', 'div_21']

    const walker = (elemObjIn, queryArrIn, iIn = 0) => {
      
      const children = elemObjIn.filter(item => item.tag === queryArrIn[iIn])[0]
        && elemObjIn.filter(item => item.tag === queryArrIn[iIn])[0].childNodes || []
      // console.info('walker [3]', { iIn, 'children.length': children.length, children, 'queryArrIn[i]': queryArrIn[iIn], queryArrIn, elemObjIn })

      if (children && children.length !== 0 && iIn < queryArr.length - 1) {
        return walker(children, queryArrIn, iIn + 1)
        // console.info('walker [10]', { iIn, 'queryArrIn[i]': queryArrIn[iIn], children, queryArrIn, elemObjIn })
      }
      return children
    }

    const result = walker(elemArr, queryArr)

    const answer =  [
      { tag: 'div_30', childNodes: [] },
      { tag: 'div_31', childNodes: [] },
      { tag: 'div_32', childNodes: [] },
    ]

    // console.info('22.2 Simulating querySelector [10]', { result, queryArr })
    expect(JSON.stringify(result)).toBe(JSON.stringify(answer))
  })
  // Create a function to select an element in the object array tree structure
  // based on array of selectors, which simulates querySelector on top level
  ...
  test('22. Algorithm for visiting the DOM element itself and all of its descendents with callback function', () => {
    ...
    const callBackFunc = (domElem) => {
      // console.info('visiting the DOM element [20]', { domElemInnerText: domElem.innerHTML })
    }

    const traverseElem2 = (elem, callback) => {

      callback(elem, '#IconCaptDesc__itemCapture')

      const children = elem.childNodes

      if (children.length > 0) {
        children.forEach(item => {
          if (item.nodeType === 1 && item.parentNode && item.innerHTML !== '') {

            return traverseElem2(item, callback)
          }
        })
      }
    }

    traverseElem2(domElem, callBackFunc)
...
describe('Section for JavaScript Interview Questions', () => {
  ...
  // How to make set and get "fangs" with private methods
  // Write a function with output: BearInst.setFangs('white')
  // expect(BearInst.getFangs()).toBe('white')
